...
## 简介
本指南面向首次接触测试管理平台的用户，目标是在最短时间内完成环境准备、安装部署与启动验证。文档覆盖以下场景：
- 环境要求：Node.js、Go语言版本与推荐IDE
- 三种部署方式：本地开发、Docker单机、Docker Compose编排、Kubernetes部署
...
图表来源
- [server/README.md:1-55](file://server/README.md#L1-L55)
- [web/README.md:1-107](file://web/README.md#L1-L107)
...
## 核心组件
- 后端核心启动入口负责初始化配置、日志、数据库、定时任务、全局处理器与表结构注册，随后启动HTTP服务。
...
### Docker 部署（单容器）
- Dockerfile 基于 CentOS 7，安装 Nginx、Go、Node、MySQL、Redis 等依赖，拷贝前端构建产物与后端配置文件，设置入口脚本
- 入口脚本在首次运行时初始化 MySQL、创建数据库与用户，启动 Redis 与 Nginx，再启动后端服务

```mermaid
flowchart TD
Start(["启动容器"]) --> CheckDB{"MySQL数据目录存在?"}
CheckDB --> |否| InitDB["初始化MySQL并创建数据库/用户"]
CheckDB --> |是| StartMySQL["启动MySQL"]
InitDB --> StartMySQL
StartMySQL --> StartRedis["启动Redis"]
StartRedis --> StartNginx["启动Nginx"]
StartNginx --> StartServer["启动后端服务"]
StartServer --> Done(["服务就绪"])
```

图表来源
- [deploy/docker/Dockerfile:1-18](file://deploy/docker/Dockerfile#L1-L18)
- [deploy/docker/entrypoint.sh:1-19](file://deploy/docker/entrypoint.sh#L1-L19)
...
- [deploy/docker/entrypoint.sh:1-19](file://deploy/docker/entrypoint.sh#L1-L19)
- [deploy/kubernetes/server/gva-server-deployment.yaml:44-65](file://deploy/kubernetes/server/gva-server-deployment.yaml#L44-L65)

## 结论
通过本指南，您可以基于本地开发、Docker 单容器或 Docker Compose 编排快速搭建测试管理平台，并在 Kubernetes 上进行生产级部署。建议优先使用 Docker Compose 进行本地联调，再迁移到 Kubernetes。遇到问题时，优先检查配置文件、服务依赖与日志输出。
