...
- [server/config/plugin.go](file://server/config/plugin.go)
...
- [server/plugins/demo-plugin/plugin.json](file://server/plugins/demo-plugin/plugin.json)
- [server/plugins/demo-plugin/menus.json](file://server/plugins/demo-plugin/menus.json)
...
## 更新摘要
**所做更改**
- 新增基于 server/plugin/manager 的完整插件架构章节
- 更新插件安装、加载、注册机制说明
- 新增钩子系统和生命周期管理章节
- 更新插件开发模式对比，从MCP工具开发转向完整插件架构
- 新增插件清单配置和菜单注册机制
- 更新示例插件结构和开发流程
...
**章节来源**
- [server/config/plugin.go:7-31](file://server/config/plugin.go#L7-L31)
- [server/plugins/demo-plugin/plugin.json:1-16](file://server/plugins/demo-plugin/plugin.json#L1-L16)
