...
- [文件存储API.md](file://repowiki/zh/content/API文档/文件存储API.md)
</cite>
...
## 简介
本文件上传处理技术文档面向测试管理平台的文件上传、下载、断点续传与多云存储集成能力，覆盖本地存储、阿里云OSS、腾讯COS、AWS S3等后端的统一接口规范。文档详细说明：
- 文件上传流程控制与分片/并发策略
- 断点续传的实现机制与目录组织
...
D["断点续传接口<br/>BreakpointContinue"]
E["查找文件接口<br/>FindFile"]
F["断点续传完成接口<br/>BreakpointContinueFinish"]
end
subgraph "业务层"
G["文件上传服务<br/>UploadFile"]
H["文件记录服务<br/>GetFileRecordInfoList"]
I["删除文件服务<br/>DeleteFile"]
J["断点续传服务<br/>FindOrCreateFile/CreateFileChunk"]
end
subgraph "工具层"
K["OSS接口抽象<br/>OSS"]
L["本地存储实现<br/>Local"]
M["阿里云OSS实现<br/>AliyunOSS"]
N["腾讯COS实现<br/>TencentCOS"]
O["AWS S3实现<br/>AwsS3"]
P["断点续传工具<br/>BreakPointContinue/MakeFile"]
end
subgraph "配置层"
Q["系统配置<br/>Server.Config"]
R["本地配置<br/>Local"]
S["阿里云配置<br/>AliyunOSS"]
T["腾讯配置<br/>TencentCOS"]
U["AWS配置<br/>AwsS3"]
end
A --> G
B --> H
...
- Element Plus上传组件
  - 支持多文件上传、限制数量、接受类型
  - 通过noSave=1实现"不入库"预览模式
  - 自动携带JWT Token头部

章节来源
- [selectFile.vue:6](file://web/src/components/selectFile/selectFile.vue#L6)
